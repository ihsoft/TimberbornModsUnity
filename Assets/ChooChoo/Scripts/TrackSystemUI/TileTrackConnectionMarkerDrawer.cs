...
namespace ChooChoo
{
  public class TileTrackConnectionMarkerDrawer : BaseComponent, ISelectionListener, IPreviewSelectionListener
  {
    ...
    {
      IBlockObjectModel blockObjectModel = _blockObjectModel;
      if ((blockObjectModel != null ? (blockObjectModel.IsModelShown ? 1 : 0) : 1) == 0)
        return;
    ...
    private void DrawTrackConnections()
    {
      ...
      {
        Vector3Int coordinates = trackConnection.Coordinates + trackConnection.Direction2D.ToOffset();
        Quaternion rotation = Quaternion.AngleAxis(trackConnection.Direction2D.ToAngle(), Vector3.up);
        Color color = _isPreview ? _colors.PreviewLandTile : _colors.BuildingLandTile;
